...
class AIS:
    def __init__(self, boat) -> None:
        ...
        """
        misc.log("Début initialisation bateau.")
        self.boat = boat
        self.antenna_87b: Antenna = Antenna(161975000, self)  # 87B
        self.antenna_88b: Antenna = Antenna(162025000, self)  # 88B
        self.slots_map: slots_map.SlotsMap = slots_map.SlotsMap(boat)
        self.boats_registry: boats_registry.BoatsList = boats_registry.BoatsList()
        self.msg_handler: message.Message = message.Message(boat, self, self.slots_map)
        ...
        self.SOTDMA_NSS: slot.Slot | None = None
        self.SOTDMA_NS: slot.Slot | None = None
        self.SOTDMA_NTS: slot.Slot | None = None
        self.SOTDMA_RI: int = 10
        self.SOTDMA_Rr: float = 60 / self.SOTDMA_RI
        self.SOTDMA_NI: int = int(misc.SLOTS_PER_MINUTE / self.SOTDMA_Rr)
        ...
        self.NO_COM_STATE_MSG_TYPES = [5, 6, 7, 8, 10, 12, 13, 14, 15, 16, 17, 19, 20, 21, 22, 23, 24, 25, 27]

        # Developer menu thread (daemon) and SOTDMA station thread
        self.dev_menu_thread: threading.Thread = threading.Thread(target=self.dev_menu, daemon=True)
        self.SOTDMA_station_thread: threading.Thread = threading.Thread(target=self.SOTDMA_station, daemon=True)
        self.dev_menu_thread.start()

        misc.log("Initialisation bateau terminée.")
    ...
    def handle_transmission(self, t: str, chn: str) -> None:
        ...
        try:
            ...
            misc.log(f"Message de type inconnu reçu et ignoré.")
    ...
    def set_initial_NSS_and_NS(self) -> None:
        ...
        """
        start_chn = random.choice(["87B", "88B"])
    ...
    def get_next_NS(self, rank: int = 0) -> slot.Slot:
        ...
        -------
        slot.Slot
            The :class:`slot.Slot` object corresponding to the computed
        ...
        AttributeError
            ...
            this AIS instance.
        """
        NS_i = int((self.SOTDMA_NSS.number + (self.SOTDMA_t_counter + rank) * self.SOTDMA_NI) % misc.SLOTS_PER_MINUTE)
        return self.slots_map.slots[NS_i]
        
        
    def set_next_NTS(self) -> slot.Slot:
        """Reserve and return the next NTS (next transmission slot).

        The algorithm scans a window of size SI around the current NS for
        free slots and chooses one at random. If no candidate is found it
        waits a short time and retries.
        """
        rsv_chn = None
        start_si = int((self.SOTDMA_NS.number - self.SOTDMA_SI // 2) % misc.SLOTS_PER_MINUTE)

        if self.SOTDMA_NTS is not None:
            rsv_chn = "87B" if self.SOTDMA_NTS.channel == "88B" else "88B"

        available_NTS = self.slots_map.scan_for_free_slots(length=self.SOTDMA_SI, ref_si=start_si, chn=rsv_chn)

        while available_NTS == []:
            time.sleep(misc.SLOTS_DURATION / 4)
            available_NTS = self.slots_map.scan_for_free_slots(length=self.SOTDMA_SI, ref_si=start_si, chn=rsv_chn)

        next_NTS = random.choice(available_NTS)
        next_NTS.book(self.boat.mmsi, timeout=random.randint(self.SOTDMA_TMO_MIN, self.SOTDMA_TMO_MAX))
        return next_NTS
        
    
    def get_next_NTS(self, rank: int = 0) -> slot.Slot | None:
        ...
        -------
        slot.Slot
            A :class:`slot.Slot` object that is owned by ``self.boat`` and
        ...
        IndexError
            ...
            (this mirrors the behaviour of :func:`random.choice`).
        """
        start_si = int((self.get_next_NS(rank).number - self.SOTDMA_SI // 2) % misc.SLOTS_PER_MINUTE)
        avail_ss = self.slots_map.scan_for_owned_slots(length=self.SOTDMA_SI, ref_si=start_si)
        try:
            return random.choice(avail_ss)
        except:
            return None
        
    
    def wait_for_slot(self, slot: slot.Slot) -> None:
    ...
    def send(self, msg_type: int, keep_flag: bool = False, offset: int = None, slots_nbr: int = 1) -> None:
        ...
        setattr(self, f"last_msg{msg_type}_timestamp", misc.get_timestamp())
        ant.send(misc.encode_string(msg))
        misc.log(f"Message {msg_type} envoyé avec succès sur le slot {self.SOTDMA_NTS}.")
        time.sleep(misc.SLOTS_DURATION)
    
    
    def SOTDMA_station(self):
        ...
        """
        misc.log(f"Début d'initialisation du SOTDMA...")
        self.SOTDMA_init()
        misc.log(f"Initialisation du SOTDMA terminée.")

        if self.SOTDMA_RI <= 120:
            misc.log(f"Entrée sur le réseau du SOTDMA...")
            self.SOTDMA_net_entry()
            misc.log(f"Entrée sur le réseau du SOTDMA terminée.")
            misc.log(f"Début de la première frame du SOTDMA...")
            self.SOTDMA_first_frame()
            misc.log(f"Fin de la première frame du SOTDMA.")
            misc.log(f"Début de la phase continue du SOTDMA.")
            while True:
                if self.last_msg5_timestamp is None or misc.get_timestamp() - self.last_msg5_timestamp >= 356:
                    self.last_msg5_timestamp = misc.get_timestamp()
    ...
    def SOTDMA_net_entry(self):
        ...
        while self.slots_map.compute_slot_offset(self.SOTDMA_NTS) > self.SOTDMA_NI:
            self.set_initial_NSS_and_NS()
            self.SOTDMA_NTS = self.set_next_NTS()
        misc.log(f"Premier NTS réservé : {self.SOTDMA_NTS}")
        self.wait_for_NTS()
    ...
    def SOTDMA_continuous(self, msg_type: int) -> None:     
        ...
        if self.get_next_NTS() is None:
            ...
            if self.SOTDMA_NTS.timeout == 0:
                start_si = int((self.SOTDMA_NS.number - self.SOTDMA_SI // 2) % misc.SLOTS_PER_MINUTE)
                available_NTS = self.slots_map.scan_for_free_slots(length=self.SOTDMA_SI, ref_si=start_si, chn=self.SOTDMA_NTS.channel)
                while available_NTS == []:
                    time.sleep(0.001)
                    available_NTS = self.slots_map.scan_for_free_slots(length=self.SOTDMA_SI, ref_si=start_si, chn=self.SOTDMA_NTS.channel)
                new_NTS = random.choice(available_NTS)
                misc.log(f"NTS {self.SOTDMA_NTS} arrivé à expiration : remplacement par le slot {new_NTS} après le prochain message.")
                ...
                self.SOTDMA_NTS = next_NTS
                new_NTS.book(self.boat.mmsi, timeout=random.randint(self.SOTDMA_TMO_MIN, self.SOTDMA_TMO_MAX))
            else:
                self.send(msg_type)
                self.SOTDMA_NTS.use()
                self.SOTDMA_t_counter += 1
    ...
    def SOTDMA_change_Rr(self, new_RI: int) -> None: # !!! UNTESTED !!!
        ...
        """
        self.wait_for_NTS()
        misc.log(f"Changement d'intervalle d'émission : passage de {self.SOTDMA_RI} à {new_RI} transmissions par minute.")
        ...
        while offset is None or offset != 0:
            self.set_next_NS()
            start_si = int((self.get_next_NS().number - self.SOTDMA_SI // 2) % misc.SLOTS_PER_MINUTE)
            avail_ss = self.slots_map.scan_for_owned_slots(length=self.SOTDMA_SI, ref_si=start_si)
            next_NTS: slot.Slot
            if avail_ss:
                next_NTS = random.choice(avail_ss)
            else:
                next_NTS = self.set_next_NTS()
            offset = self.slots_map.compute_slot_offset(next_NTS, self.SOTDMA_NTS) if self.slots_map.compute_absolute_slot_distance(next_NTS, ref_NTS) >= self.SOTDMA_SI else 0
            self.ITDMA(self.SOTDMA_NTS, 3, lme_itinc=offset, lme_itkp=True, lme_itsl=1)
    ...
    def RATDMA_slot_selection(self, chn: str, lme_rtpri: int) -> slot.Slot:
        ...
        """
        start_s = self.slots_map.current_slots(0) if chn == "87B" else self.slots_map.current_slots(1)
        lme_rtes = self.slots_map.compute_offset_slot(start_s, 150)

        candidates = self.slots_map.extract_available_slots(self.slots_map.compute_slots_range(chn, start_s.number, lme_rtes.number))
    ...
    def ITDMA(self, t_s: slot.Slot, msg_type: int, lme_itinc: int, lme_itsl: int, lme_itkp: bool) -> None:
        ...
        if msg_type in self.ITDMA_COM_STATE_MSG_TYPES:
            ...
            self.send(msg_type, offset=lme_itinc, keep_flag=lme_itkp, slots_nbr=lme_itsl)
            t_s.use()
        elif msg_type in self.NO_COM_STATE_MSG_TYPES:
            self.wait_for_slot(t_s)
            self.send(msg_type)
